...
export default function IsThisYou() {
  ...
  return (
    <section className="w-full py-16 px-6" style={{ backgroundColor: "#b90124" , marginTop:'-35px'}}>
...
{items.map((text, index) => (
  <div
    key={index}
    className="bg-white flex gap-6 p-6 rounded-md shadow-md items-start"
  >
    ...
    <p className="text-gray-800 text-lg leading-relaxed">{text}</p>
      ...
      <div className="text-center mt-8">
        <button className="bg-white text-[#b90124] font-semibold text-xl py-4 px-8 rounded-md shadow hover:opacity-90 transition">
          Have a Look at Our Proud Parents, They Have Something to Tell you
        </button>
      </div>
    </section>
  );
