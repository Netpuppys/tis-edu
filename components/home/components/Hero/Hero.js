...
function Hero() {
  ...
  // Mouse movement for bubble parallax
  useEffect(() => {
    const handleMouseMove = (event) => {
      const width = window.innerWidth || 1;
      const height = window.innerHeight || 1;
      setPosition({
        x: Math.floor((event.clientX * 100) / width),
        y: Math.floor((event.clientY * 100) / height),
      });
    };
  ...
  const movementX = !isMobile ? position.x / movementFactor : 0;
  const movementY = !isMobile ? position.y / movementFactor : 0;
  ...
  return (
    // ✅ Hero section container with fixed height
    <div
      className="hero-container-main"
      style={{
        position: "relative",
        height: "120vh", // Hero section takes full viewport height
        overflow: "hidden",
      }}
    >
      {/* ✅ HeroText overlay absolutely positioned */}
      <div
        ...
        style={{
          ...
          alignItems: "center",
          pointerEvents: "none", // Allows clicking through bubbles
          zIndex: 20,
        }}
      >
        ...
        <div className="top-relative-text-div">
          <HeroText isFilledText={!filledText} />
        </div>
      </div>

      {/* Main visual area with bubbles */}
      <div className="main-site">
        ...
        <div className="text-div-container">
          <div className="text-div">
            <h1 className="text">
              {paragraph1}
              <span className="circled-text active">
                {circledText}
                <svg
                  ref={yellowCircleRef}
                  ...
                  className="yellow-circle active"
                >
                  <path
